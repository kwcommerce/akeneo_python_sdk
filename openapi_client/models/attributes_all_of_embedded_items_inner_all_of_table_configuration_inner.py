...
class AttributesAllOfEmbeddedItemsInnerAllOfTableConfigurationInner(BaseModel):
    """
    ...
    validations: Optional[AttributesAllOfEmbeddedItemsInnerAllOfTableConfigurationInnerValidations] = None
    labels: Optional[AttributesAllOfEmbeddedItemsInnerAllOfTableConfigurationInnerLabels] = None
    is_required_for_completeness: Optional[StrictBool] = Field(default=False, description="Defines if the column should be entirely filled for the attribute to be considered complete")
    __properties: ClassVar[List[str]] = ["code", "data_type", "validations", "labels", "is_required_for_completeness"]

    @field_validator('data_type')
    def data_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['select', 'text', 'number', 'boolean']):
            raise ValueError("must be one of enum values ('select', 'text', 'number', 'boolean')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "validations": AttributesAllOfEmbeddedItemsInnerAllOfTableConfigurationInnerValidations.from_dict(obj["validations"]) if obj.get("validations") is not None else None,
            "labels": AttributesAllOfEmbeddedItemsInnerAllOfTableConfigurationInnerLabels.from_dict(obj["labels"]) if obj.get("labels") is not None else None,
