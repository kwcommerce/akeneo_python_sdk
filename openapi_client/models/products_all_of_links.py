...
class ProductsAllOfLinks(BaseModel):
    """
    ProductsAllOfLinks
    """ # noqa: E501
    var_self: Optional[ProductsAllOfLinksSelf] = Field(default=None, alias="self")
    first: Optional[ProductsAllOfLinksFirst] = None
    previous: Optional[ProductsAllOfLinksPrevious] = None
    next: Optional[ProductsAllOfLinksNext] = None
    __properties: ClassVar[List[str]] = ["self", "first", "previous", "next"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "self": ProductsAllOfLinksSelf.from_dict(obj["self"]) if obj.get("self") is not None else None,
            "first": ProductsAllOfLinksFirst.from_dict(obj["first"]) if obj.get("first") is not None else None,
            "previous": ProductsAllOfLinksPrevious.from_dict(obj["previous"]) if obj.get("previous") is not None else None,
            "next": ProductsAllOfLinksNext.from_dict(obj["next"]) if obj.get("next") is not None else None
        })
        return _obj
