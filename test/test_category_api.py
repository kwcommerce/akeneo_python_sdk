# coding: utf-8

"""
    Akeneo PIM REST API

    No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

    The version of the OpenAPI document: 1.0.0
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.api.category_api import CategoryApi


class TestCategoryApi(unittest.TestCase):
    """CategoryApi unit test stubs"""

    def setUp(self) -> None:
        self.api = CategoryApi()

    def tearDown(self) -> None:
        pass

    def test_get_categories(self) -> None:
        """Test case for get_categories

        Get list of categories
        """
        pass

    def test_get_categories_code(self) -> None:
        """Test case for get_categories_code

        Get a category
        """
        pass

    def test_get_category_media_files_file_path_download(self) -> None:
        """Test case for get_category_media_files_file_path_download

        Download a category media file
        """
        pass

    def test_patch_categories(self) -> None:
        """Test case for patch_categories

        Update/create several categories
        """
        pass

    def test_patch_categories_code(self) -> None:
        """Test case for patch_categories_code

        Update/create a category
        """
        pass

    def test_post_categories(self) -> None:
        """Test case for post_categories

        Create a new category
        """
        pass

    def test_post_category_media_files(self) -> None:
        """Test case for post_category_media_files

        Create a category media file
        """
        pass


if __name__ == '__main__':
    unittest.main()
